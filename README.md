...
## النشر المحمي بكلمة مرور (GitHub Pages)

الموقع المباشر يُنشر **مشفّراً بكلمة مرور** عبر [StatiCrypt](https://github.com/robinmoisson/staticrypt) (AES-256 + PBKDF2، فكّ التشفير في المتصفّح):

- مجلد `Serve/` = المصدر بنص صريح (للتحرير فقط).
- مجلد `_site/` = النسخة المشفّرة التي تُنشر فعلياً على Pages — **هي وحدها** ما يظهر على الموقع المباشر، لا المصدر.
- `.github/workflows/deploy-pages.yml` يرفع `_site/` إلى GitHub Pages عند كل دفع إلى `main`.
- بملح مشترك + خيار «تذكّرني»، تسجيل دخول واحد يفتح كل الصفحات.

**إعادة البناء بعد تعديل المحتوى** (كلمة المرور تُمرَّر وقت البناء فقط، لا تُحفظ في المستودع):

```bash
STATICRYPT_PASSWORD='كلمة-المرور' ./scripts/build.sh
git add _site && git commit -m "Rebuild encrypted site" && git push
```

> ⚠️ ملاحظة أمنية: ما دام المستودع عاماً، تبقى ملفات `Serve/` الأصلية مقروءة للجميع مباشرةً من المستودع؛ حماية كلمة المرور تخصّ رابط الموقع المباشر فقط. للحماية الكاملة حوِّل المستودع إلى Private.
